...
public class HierarchyRenderer<T> {
	...
	/** Constants. */
	public static final Color WIDGET_BACKGROUND = Color.WHITE;

	/** Constants. */
	public static final Color DISABLED_FOREGROUND = Color.GRAY;

	/** Constants. */
	public static final Color DISABLED_BACKGROUND = Color.decode("#E6E6E6");

	/** Constants. */
	public static final Color NORMAL_FOREGROUND = Color.BLACK;

	/** Constants. */
	public static final Color ALTERNATIVE_FOREGROUND = Color.WHITE;

	/** Constants. */
	public static final Color NORMAL_BACKGROUND = Color.decode("#E6E6E6");

	/** Constants. */
	public static final Color SELECTED_BACKGROUND = Color.YELLOW;
	...
	/** Var. */
	private final HierarchyModelGrouping<T> model;
	...
	public Dimension getMinSize() {
		...
		return new Dimension(minWidth + OFFSET, minHeight + OFFSET);
	}
	...
	public void update(Graphics gc) {
		...
		if (model.isShowIntervals()) {
			for (RenderedInterval<T> context : intervals) {
				...
				context.rectangle2 = new Rectangle(OFFSET + intervalBoundWidth, top, intervalLabelWidth,
						INTERVAL_HEIGHT);
		...
		for (int i = 0; i < groups.size(); i++) {
			...
			for (RenderedGroup<T> context : groups.get(i)) {
				...
				context.rectangle2 = new Rectangle(left + fanoutBoundWidth.get(i), top, fanoutLabelWidth.get(i),
						height);
				top += height + OFFSET;
			}
	...
	private int getRequiredBoundWidth(Graphics gc, List<?> list) {
		gc.setFont(FONT);
		int width = 0;
		for (Object elem : list) {
			width = Math.max(width,
					(int) gc.getFontMetrics().getStringBounds(((RenderedComponent<T>) elem).bounds, gc).getWidth());
		}
		return width;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private int getRequiredLabelWidth(Graphics gc, List<?> list) {
		gc.setFont(FONT);
		int width = 0;
		for (Object elem : list) {
			width = Math.max(width,
					(int) gc.getFontMetrics().getStringBounds(((RenderedComponent<T>) elem).label, gc).getWidth());
		}
		return width;
	}
	...
	private void updateGroups(int[] factors, List<HierarchyWizardGroupingInterval<T>> modelIntervals) {
		...
		for (int i = 0; i < modelGroups.size(); i++) {
			...
			if (layout.isPretty() && showIntervals && i > 0) {
				width = dtype.subtract(groups.get(i - 1).get(groups.get(i - 1).size() - 1).max,
						groups.get(i - 1).get(0).min);
			}
