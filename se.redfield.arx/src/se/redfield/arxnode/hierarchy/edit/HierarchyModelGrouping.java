...
/**
 * A base-class for grouping-based builders, i.e., order-based and
 * interval-based builders
 * 
 * @author Fabian Prasser
 *
 * @param <T>
 */
public abstract class HierarchyModelGrouping<T> extends HierarchyModelAbstract<T> {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public HierarchyModelGrouping(String[] data, DataType<T> type, boolean intervals) {
		super(data);
		this.type = type;
		...
		if (intervals) {
			...
			if (data != null) {
				...
				for (String date : data) {
					...
					if (value != null) {
						if (min == null || dtype.compare(value, min) < 0)
							min = value;
						if (max == null || dtype.compare(value, max) > 0)
							max = value;
					}
				}
				...
				this.intervals.add(
						new HierarchyWizardGroupingInterval<T>(dtype.getMinimum(), dtype.getMaximum(), this.function));
			}
		} else {
